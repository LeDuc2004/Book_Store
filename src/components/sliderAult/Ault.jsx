...
function Ault({ kids, text, iduser }) {
  ...
  function handleLeft() {
    if (togle) {
      let lists = document.querySelectorAll('.Product__item2');
      document.getElementById('slider2').prepend(lists[lists.length - 1]);
      if (tive == 0) {
  ...
  function handleRight(boolen) {
    if (boolen == true) {
      let lists = document.querySelectorAll('.Product__item2');
      document.getElementById('slider2').appendChild(lists[0]);
...
export default React.memo(Ault) ;
